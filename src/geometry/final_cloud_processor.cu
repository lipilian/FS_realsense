...
__global__ void initializeMeshKernel(const float* xyz, const std::uint8_t* valid,
                                     const std::uint8_t* mask, int mw, int mh,
                                     int w, int h, int step, float depth_threshold_m,
                                     int* cells_valid, int cells_x, int cells) {
...
__global__ void writeMeshIndicesKernel(const int* cells_valid, int w, int step,
    ...
    const int x = (cell % cells_x) * step, y = (cell / cells_x) * step;
    const int a = y * w + x, b = a + step, c = a + step * w, d = c + step;
    const int output = 6 * cell;
    if (cells_valid[cell] != 0) {
        indices[output] = a; indices[output + 1] = b; indices[output + 2] = c;
        indices[output + 3] = b; indices[output + 4] = d; indices[output + 5] = c;
    } else {
        indices[output] = indices[output + 1] = indices[output + 2] =
            indices[output + 3] = indices[output + 4] = indices[output + 5] = 0;
    }
}
